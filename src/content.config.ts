 ...
 *
 * Astro 6 Content Layer: entries are loaded with the `glob` loader, so each
 * entry's `id` is the filename without extension (the old `slug`).
 */
const parcels = defineCollection({
  loader: glob({ pattern: "**/*.md", base: "./src/content/parcels" }),
  schema: ({ image }) =>
    z.object({
      // --- Identity ---
      title: z.string(),
